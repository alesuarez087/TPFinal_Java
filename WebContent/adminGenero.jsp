...
<body>
		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					...
					<li class="active"><a href="adminGenero.jsp">Géneros<span
							class="sr-only">(current)</span></a></li>
	...
	<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
		...
		<form role="form" action="srvGenero" method="post" id="formTabla"
			name="formTabla">
			<table>
				<tr>
					<td><b>Código:</b></td>
					<td>
						<div class="form-inline">
							<input type="text" class="form-control" id="idGenero"
								name="idGenero" readonly
								value="<%if(request.getAttribute("idGenero")!=null){%><%=request.getAttribute("idGenero") %><% }%>"
								size="43">
						</div>
					</td>
				</tr>
				<tr>
					...
					<td><input type="text" class="form-control" id="descGenero"
						name="descGenero"
						value="<%if(request.getAttribute("descGenero")!=null){%><%=request.getAttribute("descGenero") %><% }%>"></td>
				</tr>
				<tr>
			...
			<div align="center">
				<input class="btn btn-success" type="submit" value="Guardar"
					id="newGenero" name="newGenero" /> <input class="btn btn-default"
       		...
       		for(Genero genero : generos){  %>
				<tr>
					<td style="vertical-align: middle"><input type="hidden"
						name="nro" id="nro" value="<%=genero.getId()%>" /><%=genero.getId()%>
					</td>
					<td style="vertical-align: middle"><input type="hidden"
						name="desc" id="desc" value="<%=genero.getDescripcion()%>" /><%=genero.getDescripcion()%>
					</td>
					<td style="vertical-olign: middle"><input type="checkbox"
						readonly disabled <%if(genero.isHabilitado()){ %> checked <%} %>>
					<td></td>
					<form role="form" action="srvGenero" method="post" id="botonera"
						name="botonera">
						<td style="vertical-align: middle"><input type="hidden"
							name="descSelect" id="descSelect"
							value="<%=genero.getDescripcion()%>" /> <input
