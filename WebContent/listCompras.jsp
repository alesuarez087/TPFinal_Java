		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					<li class="active"><a href="listCompras.jsp">Compras<span
					...
					<tbody>
						...
						<tr>
							<td style="vertical-align: middle"><input type="hidden"
								name="nro" id="nro" value="<%=venta.getId()%>" /><%=venta.getId()%>
							</td>
							<td style="vertical-align: middle">
								<% for(VentaItem ventaItem : ctrl.getAllVentaVentaItem(venta.getId())){ %>
								<input type="hidden" name="disco" id="disco"
								value="<%=ventaItem.GetItem().getTitulo()%>," /><%=ventaItem.GetItem().getTitulo()%>
								(x<%=ventaItem.getCantidad()%>)<%
						i++; if(i<ctrl.getAllVentaVentaItem(venta.getId()).size()){           			
           			%>, <% } } %>
							</td>
							<td style="vertical-align: middle"><input type="hidden"
								name="artista" id="artista" value="<%=venta.getFecha()%>" /><%=venta.getFecha()%>
							</td>
							<td style="vertical-align: middle"><input type="hidden"
								name="artista" id="artista"
								value="$ <%=Math.rint(venta.GetMonto())%>" /><%=Math.rint(venta.GetMonto())%>
							</td>
						</tr>
						<% } %>
					</tbody>
