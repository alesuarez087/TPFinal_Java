...
<body>
		...
		<div class="row">
			<div class="col-sm-3 col-md-2 sidebar">
				<ul class="nav nav-sidebar">
					...
					<li class="active"><a href="adminArtista.jsp">Artistas<span
	...
	<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
		...
		<form role="form" action="srvArtista" method="post" id="formTabla"
			...
			<table>
				<tr>
					...
					<td>
						<div class="form-inline">
							<input type="text" readonly class="form-control" id="idArtista"
								name="idArtista" readonly
								value="<%if(request.getAttribute("idArtista")!=null){%><%=request.getAttribute("idArtista") %><% }%>"
								size="43">
						</div>
					</td>
				</tr>
				<tr>
					<td><b>Nombre:</b></td>
					<td><input type="text" class="form-control" id="descArtista"
						name="descArtista"
						value="<%if(request.getAttribute("descArtista")!=null){%><%=request.getAttribute("descArtista") %><% }%>"></td>
				</tr>
				<tr>
			...
			<tbody>
				...
				<tr>
					<td style="vertical-align: middle"><input type="hidden"
						name="nro" id="nro" value="<%=artista.getId()%>" /><%=artista.getId()%>
					</td>
					<td style="vertical-align: middle"><input type="hidden"
						name="desc" id="desc" value="<%=artista.getNombre()%>" /><%=artista.getNombre()%>
					</td>
					<td style="vertical-olign: middle"><input type="checkbox"
						readonly disabled <%if(artista.isHabilitado()){ %> checked <%} %>>
					<td></td>
					<form role="form" action="srvArtista" method="post" id="botonera"
						name="botonera">
						<td style="vertical-align: middle"><input type="hidden"
							name="nombreSelect" id="nombreSelect"
							value="<%=artista.getNombre()%>" /> <input
...
<%
		...
		if(user.getTipoUsuario() == Usuario.TiposUsuario.Empleado) {
